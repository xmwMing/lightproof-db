...
const {getStartBlock, getDB} = require("./db");
const DB = getDB(); //initalize databases

const endpoint = process.env.SHIP_WS;
...
class SYNC {
    ...
    get_blocks_result_v0(response) {
        /*  if (response.traces) {
              response.traces = this.deserialize("transaction_trace[]", response.traces);
          }
          this.deserialize("signed_block", response.block);*/
